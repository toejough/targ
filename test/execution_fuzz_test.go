...
func TestFuzz_Backoff_ArbitraryParameters(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		durationMs := rapid.IntRange(-100, 100).Draw(rt, "durationMs")
		duration := time.Duration(durationMs) * time.Millisecond
		multiplier := rapid.Float64Range(-2.0, 5.0).Draw(rt, "multiplier")
...
func TestFuzz_BuilderChain_ArbitraryOrder(t *testing.T) {
	t.Parallel()
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		ops := rapid.IntRange(0, 10).Draw(rt, "numOps")
		for range ops {
			op := rapid.IntRange(0, 5).Draw(rt, "op")
			switch op {
			case 0:
				target = target.Name(rapid.String().Draw(rt, "name"))
			case 1:
				target = target.Description(rapid.String().Draw(rt, "desc"))
			case 2:
				target = target.Cache(rapid.String().Draw(rt, "cache"))
			case 3:
				target = target.Watch(rapid.String().Draw(rt, "watch"))
			case 4:
				target = target.Retry()
			case 5:
				target = target.Times(rapid.IntRange(1, 5).Draw(rt, "times"))
			}
		}
...
func TestFuzz_Deps_ArbitraryDependencies(t *testing.T) {
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		numDeps := rapid.IntRange(0, 5).Draw(rt, "numDeps")
...
func TestFuzz_While_ArbitraryPredicates(t *testing.T) {
	t.Parallel()
	...
	rapid.Check(t, func(rt *rapid.T) {
		...
		maxCalls := rapid.IntRange(0, 5).Draw(rt, "maxCalls")
		callCount := 0
