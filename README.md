...
If targ rejects an invocation as unresolvable, nothing runs. `targ bogus` reports the unknown command without running the default target or its dependencies, and the same holds for a shell-command target, for a chain like `targ build bogus`, and under `-p`.
...
Targ caches compiled binaries in `~/.cache/targ/`. The cache is invalidated when source files or `go.mod`/`go.sum` change — including local filesystem directories referenced by `replace` directives.
