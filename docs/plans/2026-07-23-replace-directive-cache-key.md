...
## CORRECTION (post final review, 2026-07-23)

The Architecture section's claim that `computeModuleCacheKey` is "the single assembly point for cache inputs" was wrong: `computeCacheKey` had a second caller, `prepareBootstrap` (single-module and isolated builds — issue #27's own repro shape), which shipped unfixed through all gates and was caught by the real-binary e2e smoke. Fixed by consolidating both paths onto one `collectCacheInputs` assembly function with a test pinned at the previously-broken path. Additionally, `filesystemReplaceDirs` now degrades with a stderr warning instead of erroring when go.mod contains directives newer than the vendored x/mod parser (a valid future-toolchain go.mod must not brick targ; raw go.mod bytes remain hashed, so the degradation is loud and limited to replace-target invalidation).
