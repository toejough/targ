...
func TestDefaultRootTargetExamples(t *testing.T) {
	t.Parallel()

	t.Run("NoCallerExamplesPrependsBareForm", func(t *testing.T) {
		t.Parallel()
		g := NewWithT(t)
		...
		// The bare form is advertised ahead of the by-name form.
		g.Expect(strings.Index(output, "Basic usage:")).
			To(BeNumerically("<", strings.Index(output, "By name:")))
	})

	t.Run("CallerExamplesWinUntouched", func(t *testing.T) {
		t.Parallel()
		g := NewWithT(t)
		...
		opts := help.TargetHelpOpts{
			...
			DefaultRoot: true,
			Examples: []help.Example{
				{Title: "Custom", Code: "app marker --flag"},
			},
			Filter: help.TargFlagFilter{IsRoot: false},
		}
		...
		g.Expect(output).To(ContainSubstring("Custom:\n    app marker --flag"))
		// The bare form must not be injected when the caller supplies examples.
		g.Expect(output).NotTo(ContainSubstring("Basic usage"))
		g.Expect(output).NotTo(ContainSubstring("By name"))
	})

	t.Run("NonDefaultRootStaysSingleExample", func(t *testing.T) {
		t.Parallel()
		g := NewWithT(t)
